...
function doRectanglesOverlap(rect1, rect2) {
  const startTop = rect1.top + rect1.height;
  const startLeft = rect1.left + rect1.width;
  const endTop = rect2.top;
  const endLeft = rect2.left;

  if (endTop - startTop < 0 && endLeft - startLeft < 0) return true;
  return false;
}
...
function getIntervalString(a, b, isStartIncluded, isEndIncluded) {
  ...
  return `${startBracket}${[a, b].sort((x, y) => x - y).join(', ')}${endBracket}`;
}
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathes[0].length; i += 1) {
    const path = pathes[0].split('/')[i];
    let isCommon = true;

    for (let j = 1; j < pathes.length; j += 1) {
      if (path !== pathes[j].split('/')[i]) {
        isCommon = false;
        break;
      }
    }
